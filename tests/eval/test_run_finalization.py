...
wrong -- and the easiest place to lose the truth. A completion failure never MASKS the error
the run already failed with, and on its own it surfaces as ONE clear error (not a double
traceback) -- and the run's real failure always wins.
"""
