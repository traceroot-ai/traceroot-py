"""Phase 3 integration closure: ONE resolver, ONE resolved value threaded through the local
...
def test_explicit_multi_metric_selection_used_everywhere():
    ...
    fake = FakeTransport()
    result = evaluate(
        name="r", dataset=_DS, task=echo, scorers=[accuracy, fmt], main_score="format", report_to=fake
    )
...
def test_missing_main_score_completes_run_terminally_then_raises():
    ...
    with pytest.raises(ValueError, match="accuracy"):  # lists the emitted metrics
        evaluate(name="r", dataset=_DS, task=echo, scorers=[acc], main_score="missing", report_to=fake)
